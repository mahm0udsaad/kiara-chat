...
function Score({ value, label }: { value: number; label: string }) {
  ...
  const tone = normalized >= 75 ? colors.success : normalized >= 50 ? colors.warning : colors.danger;
  const soft =
    normalized >= 75
      ? colors.successSoft
      : normalized >= 50
...
export default function OrderAnalysisScreen() {
  ...
  const analysis = analysisRequest.data.analysis;
  return (
    <ScrollView
    ...
    >
      ...
      <Text selectable style={{ ...type.caption, color: colors.textTertiary, ...rtlText }}>
        بُني التحليل على {analysis.basis.messages} رسالة و{analysis.basis.bookings} حجز. التحليل آلي وقد يخطئ، لذلك راجعي المحادثة قبل اتخاذ قرار.
      </Text>
