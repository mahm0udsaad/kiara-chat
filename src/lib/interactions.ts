...
/**
 * Employee takeover of a conversation another employee holds.
 *
 * The dedicated RPC verifies the caller is an active member, locks the row,
 * changes ownership and records both audit events in one transaction. Keeping
 * that operation in the database prevents two employees rescuing the same
 * absent colleague's thread at the same time.
 */
export async function takeOverConversation(input: {
  ...
  const { conversationId, session, expectedAssignee } = input;
  const reason = input.reason.trim();

  if (!session.teamMemberId) {
    throw new TakeoverError("TAKEOVER_MEMBER_REQUIRED");
  }
  if (
  ...
  const { data, error } = await supabase.rpc("take_over_conversation", {
    p_conversation_id: conversationId,
    p_expected_assignee: expectedAssignee,
    p_reason: reason,
  });
  if (error) {
    if (error.message.includes("takeover_reason_required")) {
      throw new TakeoverError("TAKEOVER_REASON_REQUIRED");
    }
    if (error.message.includes("takeover_not_needed")) {
      throw new TakeoverError("TAKEOVER_NOT_NEEDED");
    }
    if (error.message.includes("takeover_owner_changed")) {
      throw new TakeoverError("TAKEOVER_OWNER_CHANGED");
    }
